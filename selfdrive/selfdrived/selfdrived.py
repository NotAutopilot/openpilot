...
class SelfdriveD:
  ...
  def update_events(self, CS):
    ...
    if CS.canValid:
      ...
      # Track via the explicit pedalLongActive flag from carstate, not
      # cruiseState.speed (DI_digitalSpeed reads non-zero on lateral-only).
      if (self.CP.brand == "tesla"
        ...
        # Max regen warning: pedal at limit, driver should brake for more decel.
      ...
      # Tesla Pre-AP pedal-long: brake/regen drops longitudinal only, lateral stays.
    ...
    if not REPLAY:
      ...
      # Pre-AP Tesla manages cruiseState.enabled via software FSM (not hardware
      # PCM), so treat it the same as pcmCruise here.
